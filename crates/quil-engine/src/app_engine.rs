...
impl AppConsensusEngine {
            ...
            tokio::select! {
                ...
                _ = cw_retry_timer.tick() => {
                    ...
                    if cw_transport_ready && self.cw_handle.is_none() {
                        ...
                        let member_pubkeys: Vec<Vec<u8>> = members
                            .iter()
                            .map(|(public_key, _)| public_key.clone())
                            .collect();
                        let fp = Self::committee_fp(&member_pubkeys);
    ...
    fn compute_committee_members(&self) -> (u64, &'static str, Vec<(Vec<u8>, Vec<u8>)>) {
        ...
        let members = self.prover_registry
            ...
            .map(|a| {
                a.iter()
                    .map(|p| (p.public_key.clone(), p.address.clone()))
                    .collect()
            })
            .unwrap_or_default();
    ...
    fn start_consensus_cw(
        ...
        let (committee_frame, anchor_source, committee_members) = self.compute_committee_members();
        let malformed_member_addresses: Vec<String> = committee_members
            .iter()
            .filter(|(public_key, _)| public_key.len() != 897)
            .map(|(_, address)| hex::encode(address))
            .collect();
        let member_pubkeys: Vec<Vec<u8>> = committee_members
            .into_iter()
        ...
        let (scheme, peers) = match crate::cw_app_seams::build_app_committee(
            ...
            Err(reason) => {
                ...
                warn!(
                    ...
                    committee_build_detail = ?reason,
                    malformed_member_addresses = ?malformed_member_addresses,
